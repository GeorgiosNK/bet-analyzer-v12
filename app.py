...
# CALCULATIONS ENGINE
# ==============================
h_t = max(1, st.session_state.hw + st.session_state.hd + st.session_state.hl)
a_t = max(1, st.session_state.aw + st.session_state.ad + st.session_state.al)
total = h_t + a_t

# Υπολογισμός Ποσοστών για τους τίτλους
h_pos = ((st.session_state.hw + st.session_state.hd) / h_t) * 100
a_pos = ((st.session_state.aw + st.session_state.ad) / a_t) * 100

inv = (1/odd1 + 1/oddX + 1/odd2)
pm1, pmX, pm2 = (1/odd1)/inv, (1/oddX)/inv, (1/odd2)/inv

alpha = min(1.0, (st.session_state.hw + st.session_state.hd + st.session_state.hl + st.session_state.aw + st.session_state.ad + st.session_state.al) / 15)
h_wr = st.session_state.hw / h_t
a_wr = st.session_state.aw / a_t
...
# Κάλυψη βάσει οδηγιών
if pX >= 0.40:
    if p2 > p1: base = "X (X2)"
    elif p1 > p2: base = "X (1X)"
    else: base = "X"
elif odd_check >= 2.80:
