...
def get_double_chance_reason(p1, pX, p2, h_t, a_t, st_session):
    reasons = []
    if h_t > 0:
        h_l_rate = st_session.hl / h_t
        h_d_rate = st_session.hd / h_t
        if h_l_rate < 0.15: reasons.append(f"🏠 Γηπεδούχος: Μόνο {h_l_rate*100:.0f}% ήττες εντός έδρας")
        if h_d_rate > 0.35: reasons.append(f"🤝 Γηπεδούχος: {h_d_rate*100:.0f}% ισοπαλίες")
    if a_t > 0:
        a_l_rate = st_session.al / a_t
        a_d_rate = st_session.ad / a_t
        if a_l_rate < 0.20: reasons.append(f"🚀 Φιλοξενούμενος: Μόνο {a_l_rate*100:.0f}% ήττες εκτός έδρας")
        if a_d_rate > 0.35: reasons.append(f"🤝 Φιλοξενούμενος: {a_d_rate*100:.0f}% ισοπαλίες")
    return reasons
...
def calculate_key_metrics(p1, pX, p2, odd1, oddX, odd2):
    metrics = {'home_edge': p1*100 - (1/odd1*100), 'draw_edge': pX*100 - (1/oddX*100), 'away_edge': p2*100 - (1/odd2*100)}
    edges = [metrics['home_edge'], metrics['draw_edge'], metrics['away_edge']]
    metrics['value_amount'] = max(edges)
    metrics['best_value'] = list(metrics.keys())[edges.index(max(edges))]
    return metrics
...
with st.expander("🛡️ Double Chance Analysis", expanded=False):
    ...
    if dc_recommendations:
        for rec in dc_recommendations:
            b_c = "#e8f5e9" if rec['odds'] >= 1.80 else "#fff3e0" if rec['odds'] >= 1.50 else "#e3f2fd"
            st.markdown(f"""
            <div class="dc-card" style="background-color: {b_c}; border-left-color: #1e3c72;">
                <div style="display: flex; justify-content: space-between;">
                    <span style="font-size: 2rem; font-weight: bold;">{rec['pick']} ({rec['prob']:.1f}%)</span>
                    <span style="font-size: 1.8rem; font-weight: bold;">{rec['odds']:.2f}</span>
                </div>
                <div>📌 {rec['reason']}</div>
            </div>
            """, unsafe_allow_html=True)
...
with st.expander("🔍 Αναλυτική Εξήγηση Πρόβλεψης", expanded=False):
    ...
    for exp in explanations: st.markdown(f"- {exp}")
