...
# CONFIG
# ==============================
st.set_page_config(page_title="Bet Analyzer v13.0.2 VALUE PRO", page_icon="⚽", layout="centered")
...
# Alpha Factor (Stat Confidence) - Scales up to 20 games
...
p_X = max(0.05, 1 - p_1 - p_2) # Ensure min 5% draw probability
...
suffix = " (VALUE)" if best_value >= 0.05 else " (LOW CONF)"
proposal = f"{best_pick}{suffix}"

warning = ""
if best_value < 0.05:
    warning = "⚠️ ΠΡΟΣΟΧΗ: Το μοντέλο δεν εντοπίζει ισχυρό πλεονέκτημα (Edge)."
...
# Confidence Scoring
conf = int(min(100, (alpha * 50) + (max(0, best_value) * 250)))
conf = max(5, conf)
...
# ==============================
# UPDATED CHART WITH DATA LABELS
# ==============================
fig = go.Figure()
...
fig.add_trace(go.Bar(
    name='Bookmaker %',
    x=['1', 'X', '2'],
    y=[p_m1*100, p_mX*100, p_m2*100],
    marker_color='#1e3c72',
    text=[f"<b>{p_m1*100:.1f}%</b>", f"<b>{p_mX*100:.1f}%</b>", f"<b>{p_m2*100:.1f}%</b>"],
    textposition='inside',
...
fig.add_trace(go.Bar(
    name='Model %',
    x=['1', 'X', '2'],
    y=[p_1*100, p_X*100, p_2*100],
    marker_color='#2ecc71',
    text=[f"<b>{p_1*100:.1f}%</b>", f"<b>{p_X*100:.1f}%</b>", f"<b>{p_2*100:.1f}%</b>"],
    textposition='inside',
    insidetextanchor='middle',
    textfont=dict(color="white", size=14)
))
